...
def cleanup_bot_completely(bot_id, bot_token):
    """Faz uma limpeza completa de todos os recursos do bot"""
    bot_id = str(bot_id)
    
    # 1. Limpa processo se existir
    if bot_id in processes:
        try:
            ...
            processes.pop(bot_id)
            print(f"Processo {bot_id} limpo completamente")
        except Exception as e:
            print(f"Erro ao limpar processo: {e}")
    
    # 2. Limpa dados em memória
    if bot_id in bots_data:
        bots_data.pop(bot_id)
    
    # 3. Limpa conexões do Telegram
    if bot_token and not 'BANIDO' in bot_token:
        try:
            # Força logout
            requests.post(f"https://api.telegram.org/bot{bot_token}/logOut", timeout=5)
            time.sleep(1)
            ...
            # Fecha conexão
            requests.post(f"https://api.telegram.org/bot{bot_token}/close", timeout=5)
            
        except Exception as e:
            print(f"Erro ao limpar conexões Telegram: {e}")
    
    # 4. Força coleta de lixo do Python
    import gc
    gc.collect()
...
def ban_bot(bot_id):
    if session.get("auth", False):
        ...
        if bot:
            ...
            # 2. Salva o token original antes de banir
            manager.save_bot_token_backup(bot_id, bot_token)
            
            # 3. Faz limpeza completa do bot principal
            cleanup_bot_completely(bot_id, bot_token)
            
            # 4. Marca como banido no banco
            ...
            for other_bot in bots_to_ban:
                ...
                # Faz limpeza completa
                cleanup_bot_completely(other_bot_id, other_bot_token)
                
                # Marca como banido
                manager.update_bot_token(other_bot_id, f'BANIDO-{other_bot_id}')
            
            # 6. Aguarda um pouco para garantir que tudo foi limpo
            time.sleep(1)
            
            # 7. Notifica o owner via bot de registro
            if REGISTRO_TOKEN:
                try:
        ...
        try:
            ...
            for bot in all_bots:
                if str(bot[2]) == str(owner_id):
                    ...
                    # IMPORTANTE: Faz limpeza completa primeiro
                    cleanup_bot_completely(bot_id, bot_token)
            ...
            # Aguarda um pouco mais para garantir limpeza completa
            time.sleep(3)
            
            # Reinicia os bots com tentativas
            bots_restarted = 0
            failed_bots = []
            ...
            for bot_id, bot_token in bots_to_restart:
                try:
                    ...
                    bot_check = manager.check_bot_token(bot_token)
                    if not bot_check:
                        failed_bots.append(bot_id)
                        continue
                    
                    # Tenta fazer login no bot primeiro
                    try:
                        # Faz logout se necessário e depois getMe para "acordar" o bot
                        requests.post(f"https://api.telegram.org/bot{bot_token}/logOut", timeout=5)
                        time.sleep(2)
                        ...
                        if test_response.status_code != 200:
                            print(f"Bot {bot_id} não está respondendo")
                            failed_bots.append(bot_id)
                            continue
                            
                    except Exception as e:
                        print(f"Erro ao testar bot {bot_id}: {e}")
                    ...
                    # Inicia o bot
                    if start_bot(bot_token, bot_id):
                        bots_restarted += 1
                        print(f"Bot {bot_id} reiniciado com sucesso")
                    else:
                        failed_bots.append(bot_id)
            ...
            if REGISTRO_TOKEN:
                try:
                    status_msg = ""
                    if failed_bots:
                        status_msg = f"\n\n⚠️ {len(failed_bots)} bot(s) não puderam ser reiniciados.\nTente registrá-los novamente."
